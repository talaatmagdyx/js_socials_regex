...
describe('SocialExtraction', () => {
    describe('extractMatchesPerPlatform', () => {
        it('should extract matches for each platform', () => {
            const text = 'Check out my YouTube channel: https://www.youtube.com/user/username123 and follow me on Twitter: https://twitter.com/talaatmagdyx';
            ...
            expect(matches[SocialsRegex.Platforms.PLATFORM_YOUTUBE]).to.have.property('user');
            expect(matches[SocialsRegex.Platforms.PLATFORM_YOUTUBE].user).to.be.an('array').that.is.not.empty;

            expect(matches).to.have.property(SocialsRegex.Platforms.PLATFORM_TWITTER);
            expect(matches[SocialsRegex.Platforms.PLATFORM_TWITTER]).to.have.property('user');
            expect(matches[SocialsRegex.Platforms.PLATFORM_TWITTER].user).to.be.an('array').that.is.not.empty;
        });
    });
    ...
    describe('extractMatchesByPlatform', () => {
        it('should extract matches for a specific platform', () => {
            ...
            expect(matches[SocialsRegex.Platforms.PLATFORM_YELP]).to.have.property('company');
            expect(matches[SocialsRegex.Platforms.PLATFORM_YELP].company).to.be.an('array').that.is.not.empty;
        });
        ...
        it('should throw an error for an unknown platform', () => {
            ...
            expect(() => SocialExtraction.extractMatchesByPlatform(invalidPlatform, text)).to.throw(Error);
        });
    });
