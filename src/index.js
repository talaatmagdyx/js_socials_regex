...
class PlatformsRegex {
    ...
    static REGEX = {
        ...
        [SocialsRegex.Platforms.PLATFORM_STACKEXCHANGE_NETWORK]: SocialsRegex.Regexes.STACKEXCHANGE_NETWORK_URL_REGEX,
...
class SocialExtraction {
    ...
    static extractMatchesPerPlatform(text) {
        ...
        for (const [platform, regexes] of Object.entries(PlatformsRegex.REGEX)) {
            Object.assign(matches, SocialExtraction.platformMatches({ regexes, platform, text }));
        }
        return matches;
    }
    ...
    static reformatMatches(matches) {
        ...
        for (const match of matches) {
            ...
            const namedCaptures = Object.fromEntries(Object.entries(match.groups || {}).map(([name, value]) => [name, value]));
