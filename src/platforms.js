...
SocialsRegex.Regexes = {
  ANGELLIST_URL_REGEX: {
    ...
    company:
      /(?:https?:)?\/\/angel\.co\/company\/(?<company>[A-Za-z0-9_-]+)(?:\/(?<company_subpage>[A-Za-z0-9-]+))?/,
  ...
  CRUNCHBASE_URL_REGEX: {
    ...
    company:
      /(?:https?:)?\/\/crunchbase\.com\/organization\/(?<organization>[A-Za-z0-9_-]+)/,
    // http://crunchbase.com/person/karl-lorey
    person:
      /(?:https?:)?\/\/crunchbase\.com\/person\/(?<person>[A-Za-z0-9_-]+)/
  },
  ...
  FACEBOOK_URL_REGEX: {
    ...
    username:
      /(?:https?:)?\/\/(?:www\.)?(?:facebook|fb)\.com\/(?<profile>(?![A-Za-z]+\.php)(?!marketplace|gaming|watch|me|messages|help|search|groups)[A-Za-z0-9_\-.]+)\/?/,
  ...
  LINKEDIN_URL_REGEX: {
    ...
    company:
      /(?:https?:)?\/\/(?:\w+\.)?linkedin\.com\/(?<company_type>(?:company|school))\/(?<company_permalink>[A-Za-z0-9\-À-ÿ.]+)\/?/,
  ...
  MEDIUM_URL_REGEX: {
    ...
    post_of_subdomain_publication:
      /(?:https?:)?\/\/(?<publication>(?!www)[a-z-]+)\.medium\.com\/(?<slug>[a-z0-9-]+)-(?<post_id>[A-Za-z0-9]+)(?:\?.*)?/,
    ...
    user_by_id:
      /(?:https?:)?\/\/medium\.com\/u\/(?<user_id>[A-Za-z0-9]+)(?:\?.*)/
  },
  PHONE_URL_REGEX: {
  ...
  SNAPCHAT_URL_REGEX: {
    ...
    profile:
      /(?:https?:)?\/\/(?:www\.)?snapchat\.com\/add\/(?<username>[A-Za-z0-9._-]+)\/?/
  },
  ...
  STACKOVERFLOW_URL_REGEX: {
    ...
    question:
      /(?:https?:)?\/\/(?:www\.)?stackoverflow\.com\/questions\/(?<id>[0-9]+)\/(?<title>[A-Za-z0-9\-_.]+)\/?/,
  ...
  TELEGRAM_URL_REGEX: {
    ...
    profile:
      /(?:https?:)?\/\/(?:t(?:elegram)?\.me|telegram\.org)\/(?<username>[a-z0-9_]{5,32})\/?/
  },
  TWITTER_URL_REGEX: {
    // https://twitter.com/karllorey/status/1259924082067374088
    status:
      /(?:https?:)?\/\/(?:[A-Za-z]+\.)?twitter\.com\/@?(?<username>[A-Za-z0-9_]+)\/status\/(?<tweet_id>[0-9]+)\/?/,
  ...
  VIMEO_URL_REGEX: {
    ...
    video:
      /(?:https?:)?\/\/(?:(?:www\.)?vimeo\.com|player\.vimeo\.com\/video)\/(?<id>[0-9]+)/
  },
  XING_URL_REGEX: {
    ...
    profile:
      /(?:https?:)?\/\/(?:www\.)?xing\.com\/profile\/(?<slug>[A-Za-z0-9\-_]+)/
  },
  YOUTUBE_URL_REGEX: {
    // https://www.youtube.com/channel/UCxyz123456789
    channel:
      /(?:https?:)?\/\/(?:[A-Za-z]+\.)?youtube\.com\/channel\/(?<id>[A-Za-z0-9\-_]+)/,
    ...
    video:
      /(?:https?:)?\/\/(?:(?:www\.)?youtube\.com\/(?:watch\?v=|embed\/)|youtu\.be\/)(?<id>[A-Za-z0-9\-_]+)/
  },
  ...
  YELP_URL_REGEX: {
    ...
    company:
      /(?:https?:\/\/)?(?:www\.)?yelp\.com\/biz\/(?<company>[A-Za-z0-9_-]+)/
  },
